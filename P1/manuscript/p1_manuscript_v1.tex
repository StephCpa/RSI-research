...
\author{Working manuscript -- merged theory draft v1.0}
...
\begin{proposition}[Transfer non-identification]
Suppose a model class contains two latent types that induce the same law of normalized views. Then mass can be transferred between those types without changing the observed law. In particular, if an easy correct accept $E^+$ and a blind false accept $B^+$ both satisfy $W=+\one$, then the split
\[
(\lambda_{E+},\lambda_{B+})
\]
is not identified from the view law. Only the sum
\[
S_+=\lambda_{E+}+\lambda_{B+}
\]
can be identified when the rest of the model is identified. The same statement holds for $(E^-,B^-)$ and $S_-$.
\end{proposition}
...
For a concrete audit calculation, consider a Dawid--Skene component plus unanimous atoms. Given a regular component $H=0$, views are conditionally independent given $Y$, with class-specific accuracies $\alpha_v^y>1/2$. Add four atoms: easy positives and blind false accepts at $+\one$, and easy negatives and blind false rejects at $-\one$. The observed law has the form
\begin{equation}
\label{eq:dsa}
...
\label{thm:singleton}
For the aggregate singleton-block model, in the same-sign sector and away from proper algebraic exceptional sets:
\begin{enumerate}
...
For $(2,2)$ this includes $\det R_{MM}\neq0$, a nondegenerate product quadratic, and nonzero atom-moment denominators. For $(3,1)$ it includes nondegenerate line--model intersection and a degree-one common-$\rho$ gcd. For $(2,1,1)$ it includes $\det\mathcal B\neq0$ and a minimal-degree-two saturated eliminant. For $(4)$ regularity means that \emph{at least one} of the three $2|2$ flattenings has a nondegenerate corner-free-minor certificate, together with nonzero corner-recovery slopes and a nondegenerate rank-2 product decomposition. Each condition is verified nonvacuously by an exact rational witness.
...
\]
Retaining both a check-cost budget and this modular relaxation gives two knapsack constraints. For any fixed number of knapsack constraints, monotone submodular maximization admits $1-1/e-\varepsilon$ approximations \cite{kulik2013}. The oracle objective still requires gold information because membership in $B_0^+$ contains the truth label; estimating design gains from the same audit sample creates an adaptive-reuse problem.
...
The manuscript is accompanied by deterministic scripts that separate discovery from certification. The $n=4$ singleton open-set ambiguity is certified by a standalone Krawczyk artifact with an exact rational target and an Arb interval-arithmetic check. Exact rational scripts provide Jacobian-rank witnesses for all four-view block partitions, the common-$\rho$ witness for $(3,1)$, flattening-minor witnesses for $(4)$, and a literal Rabinowitsch/Gr\"obner saturation certificate for $(2,1,1)$. Exploratory multistart scans are not used as theorem evidence.
