...
r=P(Y=-1\mid W=+\one).
\]
Gold labels sampled outside $U^+$ do not identify $r$. If $n$ labels are sampled uniformly from $U^+$ and $k$ are false accepts, an exact Clopper--Pearson binomial upper confidence bound on $r$ yields a corresponding bound on $\FA$ \cite{clopper1934}.
\end{lemma}
...
\bibitem{kim2025} E.~M.~Kim, A.~Garg, K.~Peng, N.~Garg. Correlated Errors in Large Language Models. \emph{Proceedings of the 42nd International Conference on Machine Learning}, PMLR 267:30038--30066, 2025. arXiv:2506.07962.
